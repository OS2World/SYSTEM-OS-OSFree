...
struct name psd[10];
struct name basedev[150];
struct name device[150];
struct name ifs[30];
struct name run[30];
struct name call[30];
struct name set[150];
    ...
    if(count==strlen(options_list[i])&& toupper(line[count])=='=') {
      ...
      /*  case 7: // DEVINFO_VIO
        ...
        case 56:// DEBUGPRCMGR
          ...
          break;
      default:
          kprintf("Oh, well, this should not occur. Please note down the following to lines:\n%s\n%s\n",options_list[i],line);
          break;
      }
      return(1);
    }
