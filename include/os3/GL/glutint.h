...
#if defined(__OS2__)
typedef void (GLUTCALLBACK *GLUTcommandCB) (HWND hwnd,  MPARAM mp1, MPARAM mp2 );
#endif
...
struct _GLUTwindow {
  ...
  int pf;               /* pixel format */
  HDC hdc;
  HDC renderDc;         /* device context for rendering. */
  HPS  hpsBuffer;
  Window frame;         /* frame window for client win window */
  int buttonDownState;  /* 1,2 or 3 if GLUT_LEFT_BUTTON,GLUT_MIDDLE_BUTTON or GLUT_RIGHT_BUTTON button is down */
#elif defined(_WIN32)
  int pf;               /* Pixel format. */
...
#if defined(__OS2__)
  GLUTcommandCB    wm_command;   /* WM_COMMAND  */
#endif
...
#if defined(__OS2__)
  extern void __glutOpenOS2Connection(char* display);
#elif !defined(_WIN32)
...
#if defined(__OS2PM__)
extern Bool __glutSetWindowText(Window window, char *text);
#endif
...
#if defined(__OS2__)
/* private routines from WarpWin.c */
extern HDC XHDC;
extern HWND XHWND;
