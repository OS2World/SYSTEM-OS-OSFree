...
void
gdImageDrawText(gdImagePtr im, gdFontPtr f,
              int x, int y, unsigned char *s, int color, int flCmd )
{
  int i;
  int l, lx,ly,shiftx=0,shifty=0;
  l = strlen ((char *) s);
  lx = l * f->w;
  ly = f->h;

  if(flCmd & gdDT_CENTER)  shiftx = -lx/2;
  if(flCmd & gdDT_RIGHT)   shiftx = -lx;
  if(flCmd & gdDT_VCENTER) shifty =  ly/2;
  if(flCmd & gdDT_BOTTOM)  shifty =  ly;
  if(flCmd & gdDT_TOP)     shifty =  0;


  for (i = 0; (i < l); i++)
    {
      gdImageChar (im, f, x+shiftx, y+shifty, s[i], color);
      x += f->w;
    }
}
...
void
gdImageDrawTextUp (gdImagePtr im, gdFontPtr f,
                int x, int y, unsigned char *s, int color, int flCmd )
{
  int i;
  int l, lx,ly,shiftx=0,shifty=0;
  ...
  if(flCmd & gdDT_CENTER)  shiftx = -lx/2;
  if(flCmd & gdDT_RIGHT)   shiftx = -lx;
  if(flCmd & gdDT_VCENTER) shifty =  -ly/2 + f->w;
  if(flCmd & gdDT_BOTTOM)  shifty =  -ly;
  if(flCmd & gdDT_TOP)     shifty =  0;

  for (i = 0; (i < l); i++)
    {
      gdImageCharUp (im, f, x+shiftx, y+shifty, s[i], color);
#if gdNORMALIMAGEORIGIN
      y += f->w;
#else
      y -= f->w;
