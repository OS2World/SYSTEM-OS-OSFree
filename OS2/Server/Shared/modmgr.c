                            ...
                            unsigned long * phmod)
{
  ...
  *phmod=0;
  ...
  if (!strcasecmp(pszModname, "EMXWRAP"))
  {
    if (options.debugmodmgr) io_printf(__FUNCTION__": EXMWRAP module replaced by SUB32 module.\n");
    pszModname="SUB32";
  }

  if (options.debugmodmgr) io_printf(__FUNCTION__": Loading module %s...\n", pszModname);
  ...
  // @todo extract filename only because can be fullname with path
  new_module_el = ModRegister(pszModname, ixfModule);
