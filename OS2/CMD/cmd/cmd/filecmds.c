...
static int FileCopy(char *, char *, unsigned int, long *);

static int delete_fcb(unsigned int);
static int parse_fcb(unsigned int, unsigned int);
...
int copy_cmd( int argc, char **argv )
{
        ...
        Copy.lFilesCopied = Copy.lAppendOffset = 0L;
...
static int _copy(COPY_STRUCT *Copy)
{
        ...
        for ( fval = FIND_FIRST; ; fval = FIND_NEXT ) {
                ...
                if (( Copy->fOptions & ( COPY_UPDATED | COPY_CHANGED | COPY_REPLACE )) && ( find_file( FIND_FIRST, Copy->szTargetName, 0x2107, &(Copy->t_dir), NULL ) != NULL )) {
                ...
                if (( Copy->fOptions & COPY_NOTHING ) == 0 ) {
                        ...
                        if ((( rval = FileCopy( Copy->szTargetName, Copy->szSourceName, Copy->fFlags, &( Copy->lAppendOffset ))) == CTRLC ) || ( cv.exception_flag ) || ( setjmp( cv.env ) == -1 )) {
...
next_copy:
        ...
        if ( Copy->fOptions & COPY_SUBDIRS ) {
                ...
                for ( fval = FIND_FIRST; ( find_file( fval, Copy->szSource, (mode | 0x310 ), &dir, NULL ) != NULL ); fval = FIND_NEXT) {
...
static int _mv(MOVE_STRUCT *Move)
{
        ...
        for ( fval = FIND_FIRST; ; fval = FIND_NEXT) {
                ...
                if (( Move->fFlags & ( MOVE_UPDATED | MOVE_CHANGED | MOVE_REPLACE )) && ( find_file( FIND_FIRST, Move->szTargetName, 0x2107, &( Move->t_dir), NULL ) != NULL )) {
                ...
                if (( Move->fFlags & MOVE_NOTHING ) == 0 ) {
                        ...
                        if ( rval != 0 ) {
                                ...
                                if ((( rval = FileCopy( Move->szTargetName, Move->szSourceName, fCopyFlags, &lAppendOffset )) == CTRLC ) || ( setjmp( cv.env ) == -1 )) {
        ...
        if ( Move->fFlags & MOVE_SUBDIRS ) {
                ...
                //   tell find_file() not to display errors, and to return
                //   only subdirectory names
                for ( fval = FIND_FIRST; ( find_file( fval, Move->szSource, (mode | 0x310 ),&dir,NULL ) != NULL ); fval = FIND_NEXT) {
...
static int FileCopy( char *szOutputName, char *szInputName, unsigned int fFlags, long *lAppendOffset )
{
        int rval;
                ...
                if ( *lAppendOffset != 0L )
                    ...
                    do {
                        ...
                        if (( fptr = memchr( segptr, 0x1A, nBytesRead )) != 0L ) {
                                ...
                                *lAppendOffset += ((unsigned long)fptr - (unsigned long)segptr);
                                _lseek( nOutputFile, *lAppendOffset, SEEK_SET );
                                break;
                        }
...
Del_Next:
        ...
        if ( Del->fFlags & DEL_SUBDIRS ) {
                ...
                //   return subdir names
                for ( fval = FIND_FIRST; ( find_file( fval, Del->szSource, (mode | 0x310 ), &dir, NULL ) != NULL ); fval = FIND_NEXT) {
...
int touch_cmd( int argc, char **argv )
{
        ...
        for ( argc = 0; (( arg = ntharg( argv[1], argc | 0x800 )) != NULL ); argc++ ) {
                ...
                for ( fval = FIND_FIRST; ( fval != 0 ); fval = FIND_NEXT ) {
                        ...
                        if ( find_file( fval, arg, (mode | 0x100 | FIND_BYATTS | FIND_DATERANGE ), &dir, szTarget ) == NULL ) {
                        ...
                        if ( fQuiet == 0 )
                                printf( "%s %02d:%02d:%02d  %s\n", FormatDate( sysDateTime.month, sysDateTime.day, sysDateTime.year ), sysDateTime.hours, sysDateTime.minutes, sysDateTime.seconds * 2, szTarget );

                        if ((( rc =  SetFileDateTime( szTarget, 0, &sysDateTime, fField )) != 0 ) && (( mode & 0x100 ) == 0 ))
                ...
                for ( fval = FIND_FIRST; ; fval = FIND_NEXT ) {
                        ...
                        if ( QueryIsDevice( source )) {
                            ...
                            if ( stricmp( source, CLIP ) == 0 ) {
                                ...
                                if (( pName == NULL ) || ( stricmp( pName, "NUL" ) != 0 ))
                        ...
                        } else if ( find_file( fval, source, (0x07 | FIND_BYATTS | FIND_DATERANGE ), &dir, gszCmdline ) == NULL ) {
                                if ( fval == FIND_FIRST )
                                        rval = ERROR_EXIT;
                                break;
                        }
                        ...
                        if (( fType & TYPE_LINENO ) || ( gnPageLength )) {
                                ...
                                for ( lRow = 1; ( getline( gnGFH, gszCmdline, CMDBUFSIZ-1, EDIT_DATA ) > 0 ); lRow++ ) {
                                        if ( fType & TYPE_LINENO )
                                                printf( "%4lu : ",lRow);
                                        more_page( (char *)gszCmdline, col );
                                }

                        } else {

                                while (( size = _read( gnGFH, gszCmdline, 512 )) > 0 ) {
