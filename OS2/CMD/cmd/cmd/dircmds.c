...
static LONGLONG tree_size;
static LONGLONG tree_alloc;
...
static LONGLONG total;                  // total of file sizes
static LONGLONG lTotalAllocated;        // total amount of disk space actually used
...
void init_dir(void)
{
        ...
        glDirFlags = 0L;
        dir_columns = 1;
        dir_mode = 0x10;        // default to normal files + directories
...
{
        char *arg, *ptr;
        char filename[MAXFILENAME], fname[MAXFILENAME];
        char szCPBuffer[592];
        int n, rval;
        ...
        if ( entries > 0 ) {
                ...
                for ( n = 0, last_entry = 0, v_rows = v_offset = 0; ( n < entries ); n++ ) {
                        ...
                        if ( DiskInfo.ClusterSize != -1L ) {
                            ...
                            if (( fIFS == 1 ) && ( glDirFlags & DIRFLAGS_HPFS ) && ( files[n].file_size == 0L )) {
                                ...
                                if (( stricmp( files[n].hpfs_name, "." ) != 0 ) && ( stricmp( files[n].hpfs_name, ".." ) != 0 ))
                                    lTotalAllocated += DiskInfo.ClusterSize;
                            } else
                                if ( files[n].file_size > 0L ) {
                                    lTotalAllocated += (ULONG)(( files[n].file_size + ( DiskInfo.ClusterSize - 1 )) / DiskInfo.ClusterSize ) * DiskInfo.ClusterSize;
                                }
                        }
                        ...
                        if ( glDirFlags & DIRFLAGS_ALLOCATED )
                                files[i].file_size = ((ULONG)( files[i].file_size + ( DiskInfo.ClusterSize - 1 )) / DiskInfo.ClusterSize ) * DiskInfo.ClusterSize;
                        ...
                        if ( glDirFlags & DIRFLAGS_FULLNAME ) {
                            ...
                            if ((( glDirFlags & DIRFLAGS_NAMES_ONLY ) == 0 ) || ( uColumns > 1 )) {
                                ...
                                if ( uColumns <= 2 ) {
                                    nCPLength += sprintf( pszCPBuffer+nCPLength, ( files[i].attribute & _A_SUBDIR) ? DIR_LABEL : "%9lu", files[i].file_size );
                                    nCPLength += sprintf( pszCPBuffer+nCPLength, "  %s ",(( files[i].fd.ufDate == 0 ) ? "        " : FormatDate( files[i].fd.file_date.months,files[i].fd.file_date.days,files[i].fd.file_date.years+80 )));
...
// FIXME - change to INT64??
                                    ...
                                    if ( files[i].attribute & _A_SUBDIR )
                                        ...
                                        fsize = (ULONG)files[i].file_size;

                                        // if between 1 & 10 Mb, display
                                        //   decimal part too
                                        if (( files[i].file_size >= 1048576L ) && ( files[i].file_size < 10485760L )) {
                                            fsize /= 1048576L;
                                            sprintf( pszCPBuffer+nCPLength, " %lu.%luM", fsize, (( (ULONG)files[i].file_size % 1048576L ) / 104858L ));
                                        } else {
                                            for ( ptr = DIR_FILE_SIZE; (( fsize = (( fsize + 1023 ) / 1024L )) > 999 ); ptr++ )
                                                ;
                                            sprintf( pszCPBuffer+nCPLength, "%4lu%c", fsize, *ptr );
                                        }
                                    }
                                }
                            ...
                            if (( uColumns == 1 ) && ( glDirFlags & DIRFLAGS_WIDE ) == 0 ) {
                              ...
                              if (( glDirFlags & DIRFLAGS_NAMES_ONLY ) == 0 ) {
                                ...
                                // date
                                nCPLength = sprintf( pszCPBuffer, (( files[i].fd.ufDate == 0 ) ? "        " : FormatDate( files[i].fd.file_date.months,files[i].fd.file_date.days,files[i].fd.file_date.years+80 )));

                                // time
                                nCPLength += sprintf( pszCPBuffer+nCPLength, (( files[i].fd.ufDate == 0 ) ? "        " : "  %2u%c%02u%c" ),files[i].ft.file_time.hours,gaCountryInfo.szTimeSeparator[0],files[i].ft.file_time.minutes,files[i].ampm);

                                // size or <DIR>
                                nCPLength += sprintf( pszCPBuffer+nCPLength, ( files[i].attribute & _A_SUBDIR) ? HPFS_DIR_LABEL : "%15Lu", files[i].file_size );
                                // size of EA list
                                nCPLength += sprintf( pszCPBuffer+nCPLength, "%12Lu  ", files[i].ea_size );
                              }
        ...
        if ((( glDirFlags & DIRFLAGS_NO_FOOTER ) == 0 ) && ((( uFilesDisplayed + uDirsDisplayed ) > 0 ) || (( glDirFlags & DIRFLAGS_RECURSE ) == 0 ))) {
                ...
                i = printf( DIR_BYTES_IN_FILES, total, (long)uFilesDisplayed, (( uFilesDisplayed == 1 ) ? ONE_FILE : MANY_FILES ), (LONG)uDirsDisplayed, ((uDirsDisplayed == 1) ? ONE_DIR : MANY_DIRS ));

                // if not a server sharename, display allocated & free space
                if (( current[0] != '\\' ) && ( uFilesDisplayed > 0 ))
                        printf( DIR_BYTES_ALLOCATED, lTotalAllocated );
        ...
        if ( glDirFlags & DIRFLAGS_RECURSE ) {
                ...
                if ( SearchDirectory((( dir_mode & 0xFF ) | 0x210 ), current, (DIR_ENTRY **)&files, &entries, &aRanges, 0 ) != 0 ) {
                        rval = ERROR_EXIT;
                        goto do_dir_bye;
                }
                ...
                if ((( glDirFlags & DIRFLAGS_NO_FOOTER) == 0 ) && (( ulTreeFiles + ulTreeDirs) > (t_files + t_dirs)) && (entries > 0 )) {
                        ...
                        i = printf( DIR_BYTES_IN_FILES, t_size, t_files, ((t_files == 1) ? ONE_FILE : MANY_FILES ), t_dirs, ((t_dirs == 1) ? ONE_DIR : MANY_DIRS ));
                        if ( DiskInfo.ClusterSize != -1L )
...
int SearchDirectory( int attrib, char *arg, DIR_ENTRY **hptr, unsigned int *entries, RANGES *paRanges, int nFlags )
{
        ...
        for ( fval = FIND_FIRST; ; fval = FIND_NEXT) {
                ...
                if ( find_file( fval, arg, (unsigned int)(attrib | 0x4100 ), &dir, NULL ) == NULL ) {
