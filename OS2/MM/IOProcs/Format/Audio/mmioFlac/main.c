...
/*
	pmmioprocMp3IOProc = mmioInstallIOProc(FOURCC_FLAC, IOProc_Entry, MMIO_INSTALLPROC);
...

	{
		...
		}
		{
	   		...
	   		memset(&mciloadparms, 0, sizeof(mciloadparms));
     		{
		   		...
		   		mmioinfoSS.fccIOProc = mmioFOURCC('f','L','a','C');
		   		mmioinfoSS.ulTranslate = MMIO_TRANSLATEHEADER | MMIO_TRANSLATEDATA;
  			  	mciloadparms.pszElementName = (PSZ)mmioOpen("J:\\FOO.FLAC", &mmioinfoSS, MMIO_READ);
				printf("open okay: %d\n", (HMMIO)mciloadparms.pszElementName);
			}
...
//	rc = mciSendString("open waveaudio alias wave wait", retmsg, 1024, 0, 0);
	rc = mciSendString("open J:\\FLAC\\TEST1.FLA  type WAVEAUDIO alias wave wait", retmsg, 1024, 0, 0);
	mciGetErrorString(rc, reterr, 1024);
	printf("%s : %s\n",retmsg,reterr);
