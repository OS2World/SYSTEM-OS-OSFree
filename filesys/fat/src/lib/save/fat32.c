...
USHORT rc;
   ...
   if (f32Parms.fMessageActive & LOG_FS)
      Message("FS_FLUSBUFF, flag = %d", usFlag);
...
FS_FLUSHEXIT:
   ...
   if (f32Parms.fMessageActive & LOG_FS)
      Message("FS_FLUSH returned %u", rc);
   return rc;
}
...
FS_IOCTLEXIT:
   if (f32Parms.fMessageActive & LOG_FS)
      Message("FS_FSIOCTL returned %u", rc);
   return rc;
...
USHORT rc;
   ...
   rc = FSH_QSYSINFO(2, (PBYTE)pProcInfo, usSize );
   if (rc)
...
static PBYTE pYieldFlag = NULL;
   ...
   if (!pYieldFlag)
      DevHelp_GetDOSVar(DHGETDOSV_YIELDFLAG, 0, &pYieldFlag);
   if (*pYieldFlag)
      DevHelp_Yield();
}
