/* $Id: xmkfs.c,v 1.4 2004/03/21 04:10:39 bird Exp $ */
...
*/
  ...
  if ( logdev[0] != '\0' )
  {
    ...
    if ( rc != 0 );
    {
      ...
      message_user(MSG_OSO_FORMAT_FAILED, NULL, 0, STDOUT_CODE, NO_RESPONSE, OSO_MSG);
      DBG_ERROR(("Internal error: (log_form) Format failed rc=%x\n",rc))
  ...
  else
  {
    ...
    if ( logsize >= (number_of_blocks / MAX_LOG_PERCENTAGE) )
    {
      ...
      ujfs_redeterminemedia(dev_handle);
      message_user(MSG_OSO_FORMAT_FAILED, NULL, 0, STDOUT_CODE, NO_RESPONSE, OSO_MSG);
      DBG_ERROR(("Internal error: Format failed (logsize) rc=%x\n",rc))
  ...
  if( do_lvm_BdBlks == TRUE ) {
    ...
    if ( rc != 0 )
    {
      ujfs_redeterminemedia(dev_handle);
      message_user(MSG_OSO_FORMAT_FAILED, NULL, 0, STDOUT_CODE, NO_RESPONSE, OSO_MSG);
      DBG_ERROR(("Internal error: Format failed (log_form2) rc=%x\n",rc))
  ...
  {
    message_user(MSG_OSO_FORMAT_FAILED, NULL, 0, STDOUT_CODE, NO_RESPONSE, OSO_MSG);
    DBG_ERROR(("Internal error: Format failed (redeterminatemedia) rc=%x\n",rc))
  }
  ...
  {
    message_user(MSG_OSO_FORMAT_FAILED, NULL, 0, STDOUT_CODE, NO_RESPONSE, OSO_MSG);
    DBG_ERROR(("Internal error: Format failed (DosQueryFSInfo) rc=%x\n",rc))
  }
  else
  {
