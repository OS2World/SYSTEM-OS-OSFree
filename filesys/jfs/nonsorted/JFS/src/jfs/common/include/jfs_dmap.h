/* $Id: jfs_dmap.h,v 1.1.1.1 2003/05/21 13:35:48 pasha Exp $ */

/* static char *SCCSID = "@(#)1.11  7/30/98 14:05:49 src/jfs/common/include/jfs_dmap.h, sysjfs, w45.fs32, 990417.1";
 *
...
typedef struct {
	...
	int32		dn_maxlevel;	/* 4: number of active ags 	     */
...
#define BLKTOCTLLEAF(b,m)	\
	leafno = (((b) & (((int64)1 << ((m) + L2LPERCTL)) - 1)) >> (m))
...
int32 dbUnmount(
	inode_t *ipbmap, uint32	mounterror);
...
int32 dbExtendFS(inode_t *ipbmap, int64 blkno, int64 nblocks);
int32 dbFinalizeBmap(inode_t *ipbmap);
