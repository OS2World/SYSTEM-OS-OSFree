/* $Id: jfs_readdir.c,v 1.2 2003/09/21 08:42:59 pasha Exp $ */
...
{	
	...
	uint32	position=0;		/* offsets in output */
	...
	component_t lastmatch ={0,0};	/* pointer to last matching entry PS 28092001 */
       uint64 lastoffset;          //PS
	char *ffbuf;			/* output buffer pointer */
	char *nxbuf;			/* output buffer pointer */
	char *bufp;			/* output buffer pointer */
       uint32 TempUbytes;          //PS 28092001
   ...
   MMPHPrereaddir();      /* MMPH Performance Hook */
	...
	matches = *matchcnt;
	*matchcnt = 0;
//      lastmatch.namelen=ubytes;
       TempUbytes=ubytes; //PS 28092001
...
//PS End
		...
		while ((*matchcnt < matches) && (ReturnCode == NO_ERROR))
		 	...
		 	*/
//PS 28092001			if ((count + ffhdsize + namlen + 1) > ubytes)
			if (( ffhdsize + namlen + 1) > TempUbytes)
			{
...
//PS End
		 	...
		 	*/
			ICACHE_LOCK();
			rc = iget(dvp->v_vfsp, dbuf->d_ino, &ip, 0);
			ICACHE_UNLOCK();
			 ...
			 */
//PS 28092001			rc = get_fileinfo(ip, &nxbuf, ubytes, dbufp->d_name,
			rc = get_fileinfo(ip, &nxbuf, TempUbytes, dbufp->d_name,
...
//PS 28092001				rc = get_fileinfo(ip, &nxbuf, ubytes,
				rc = get_fileinfo(ip, &nxbuf, TempUbytes,
						  dbufp->d_name, namlen,
			...
			if (rc == 0)
			{
				...
				count += nxbuf - ffbuf;
				TempUbytes -= nxbuf - ffbuf;  //PS 28092001
...
try_next:
			...
			lastmatch.namlen = namlen;
                     lastoffset=dbufp->d_offset;                  //PS 28092001
...
//PS 28092001		if (fsfp->fsd_offset == -1)
		if ( (tbytes == 0) && (fsfp->fsd_offset == -1) )
			rc = ERROR_NO_MORE_FILES;
//PS 28092001		else if ((rc == 0) || (rc == ERROR_EAS_DIDNT_FIT))
		else if (  (rc == 0) || (rc == ERROR_EAS_DIDNT_FIT) || ( rc == ERROR_BUFFER_OVERFLOW) ) 
 		       { 
                     if(lastmatch.name )
                       {
			/* save last matching name for next call */
			   ...
			   fsfp->fsd_lastmatch[lastmatch.namlen] = '\0';
                        fsfp->fsd_offset=lastoffset;   //PS 28092001
	            	   } 
                     }
	}
