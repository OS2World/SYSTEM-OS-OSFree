/* $Id: jfs_cachemgr.c,v 1.2 2003/09/21 08:42:52 pasha Exp $ */
...
cmInit()
{
	...
	nBuffer = CacheSize / PSIZE; //PS 28092001
...
#endif	/* _JFS_OS2 */
//PS 28092001  	nBuffer = CacheSize / PSIZE;
   ...
   MMPHPrecmRead();        /* MMPH Performance Hook */
	...
	if (ratail)
	{
		...
		if (wait_read)
		{
			...
			{
				rc = (rahead->cm_iobp->b_flags & B_ERROR) || (rahead->cm_free) ?  //PS21092003
					EIO : 0;
				IOCACHE_UNLOCK(ipl);
			}
			if (rc)
//PS21092003 Begin			{					// D228362
         ...
         cbuf_t **cpp)
{
   ...
   MMPHPrecmReadWR();                  /* (d201828) Performance trace hook */
	    ...
	    nbytes < (xlen << ip->i_ipmnt->i_l2bsize))
	{
		...
		{
			rc = (cp->cm_iobp->b_flags & B_ERROR) || (cp->cm_free) ? EIO : 0; //PS21092003
			IOCACHE_UNLOCK(ipl);
		}
	...
	int32	*ipl)
{
     ...
     if (cachemgr.nfreecbufs + cachemgr.numiolru <= cachemgr.minfree)
	{
		...
		nb = cachemgr.maxfree - cachemgr.nfreecbufs -
						cachemgr.numiolru;
              nb1=cachemgr.nfreecbufs + cachemgr.numiolru;   //PS01012003
...
//PS01012003		     i++, cp = cachemgr.cachelist.head)
		     cp = cachemgr.cachelist.head)
		{
              if( cachemgr.nfreecbufs + cachemgr.numiolru > nb1 )
                 {
                  i++;
                  nb1=cachemgr.nfreecbufs + cachemgr.numiolru;
                 }
//PS End                 
			/* remove the cbuf from the cachelist */
#ifdef	_JFS_HEURISTICS_SLRU
      ...
      int32  dirty)
{
   ...
   MMPHPrecmPut();                     /* (d201828) Performance trace hook */
	...
	if (dirty)
		 ...
		 */
//PS 28092001		if (cp->cm_modified == FALSE)
// cm_modified can be set in jfs_extent without move to head dirty's segment
//		{
...
//PS 28092001 begin
         ...
         if  ( (cachemgr.lazywait != EVENT_NULL) && (cachemgr.nfreecbufs >= cachemgr.minfree) )  //PS21092003
             {
              IOCACHE_WAKEUP(&cachemgr.lazywait);                                                //PS21092003
             }
#endif
//PS 28092001 end
...
//PS 28092001        		IOCACHE_UNLOCK(ipl);
//			return;
			continue;
		} /* cm_discard */
...
void rawStartIO(cbuf_t *bp)
{
	...
	/* rlhp->Count = 1; */
	/* rlhp->Notify_Address = (void *)jfsIODone; */
	/* rlhp->Request_Control = RLH_Single_Req | RLH_Notify_Err | RLH_Notify_Done; */
