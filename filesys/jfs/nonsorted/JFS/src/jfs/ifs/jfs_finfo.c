/* $Id: jfs_finfo.c,v 1.2 2003/09/21 08:42:55 pasha Exp $ */
...
{
        int32   rc;
        struct timestruc_t  TmpMtime;          //PS21092003
	...
	TmpMtime = ip->i_mtime;      //PS21092003 save current time
	...
	if( ((fsts->attrFile << ATTRSHIFT) & ~IRASH ) != 0 ) 
        ...
        /* If mtime is different, set arc bit */
        if(ip->i_mtime.tv_sec != TmpMtime.tv_sec)          //PS21092003
                ip->i_mode |= IARCHIVE;                    //PS21092003
