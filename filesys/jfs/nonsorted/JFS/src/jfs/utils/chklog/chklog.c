/* $Id: chklog.c,v 1.1.1.1 2003/05/21 13:41:00 pasha Exp $ */

static char *SCCSID = "@(#)1.5  2/27/99 12:39:18 src/jfs/utils/chklog/chklog.c, jfsutil, w45.fs32, 990417.1";
/*
 ...
 *
 *      This routine displays the contents of the specified (or implied)
 *      chkdsk service log on the specified device.  The output is sent
 *      to standard output, and can be piped into a file for sorting,
 *      searching, printing, or whatever.
 * 
 *      USAGE:  
 *
 *         chklog [-L[:N|:P]] Device
 ...
 *      SAMPLE INVOCATIONS:
 *
 *         To display the most recent log on f:        chklog f:
 *
 *         To display the most recent log on f:        chklog f: -L
 *
 *         To display the most recent log on f:        chklog f: -l:N
 *
 *         To display the prior log on f:              chklog f: -L:p
 *
 *   FUNCTIONS:
 *		main
 *
...
char fscklog_buffer[FSCKLOG_BUFSIZE];
  ...
  *
  *    values are assigned by the main chklog routine
  */
...
retcode_t  main( int32 argc, char **argv )
{
  ...
  local_recptr = &chklog_record;

  rc = initial_processing( argc, argv );

  if( rc == FSCK_OK ) {  /* we have a buffer and the device is open */

    display_service_log();
    }

  if( rc == FSCK_OK ) {  /* no errors yet */
    rc = final_processing();
    }  /* end no errors yet */
 ...
 *      failure: something else
 */
void display_service_log( )
{
  retcode_t dsl_rc = FSCK_OK;
  int       prmidx;
  char     *log_entry;
  int       log_entry_pos;
  int       bytes_left;
  ...
  while( (!local_recptr->fscklog_last_read) && 
    ...
    while( (!local_recptr->this_buffer_last_message) && 
      ...
      hdptr = (logent_hdptr) 
                 ( (int32) local_recptr->fscklog_buf_ptr + 
                   (int32) local_recptr->fscklog_buf_data_len
                  );
      if( hdptr->entry_length == 0 ) {  /* no more entries in this buffer */
        ...
        log_entry_pos = sizeof(fscklog_entry_hdr_t);
             ...
             */
        bytes_left = local_recptr->fscklog_buf_length -
                     local_recptr->fscklog_buf_data_len;
        if( (bytes_left < 0 )           ||
          ...
          if( (hdptr->msg_num < 0)                          || 
            ...
            if( hdptr->num_parms != 0 ) {  /* there are parms */
              ...
              while( prmidx < hdptr->num_parms ) {
                ...
                * issue the message
                */
            send_msg( hdptr->msg_num, hdptr->num_parms );
            }  /* end the the record looks ok */

             /* 
              * set up for the next record 
              */
          local_recptr->fscklog_buf_data_len += hdptr->entry_length;
          if( local_recptr->fscklog_buf_data_len >= 
                       local_recptr->fscklog_buf_length ) {
            local_recptr->this_buffer_last_message = -1;
            }
...
retcode_t  fscklog_fill_buffer ( )
{
  ...
  io_rc = readwrite_device( local_recptr->fscklog_agg_offset,
                            local_recptr->fscklog_buf_length,
                            &bytes_read,
                            (void *) local_recptr->fscklog_buf_ptr,
                            fsck_READ
                             );

  if( (io_rc != FSCK_OK) ||
      (bytes_read != (uint32) local_recptr->fscklog_buf_length) )  {  /*
    ...
    msgprmidx[2] = 0;
    sprintf( message_parm_3, "%lld", local_recptr->fscklog_agg_offset );
    msgprms[3] = message_parm_3;
    msgprmidx[3] = 0;
    sprintf( message_parm_4, "%ld", local_recptr->fscklog_buf_length );
    ...
    send_msg( fsck_ERRONLOG, 6 );
       ...
       * readable. 
       */
  local_recptr->fscklog_agg_offset += local_recptr->fscklog_buf_length;
  local_recptr->fscklog_log_offset += local_recptr->fscklog_buf_length;
  local_recptr->fscklog_buf_data_len = 0;

  log_bytes_left = (local_recptr->ondev_fscklog_byte_length / 2) -
                   local_recptr->fscklog_log_offset;
  if( log_bytes_left < local_recptr->fscklog_buf_length ) {  /* 
...
retcode_t initial_processing ( int32 argc, char **argv )
{
  ...
  /*
   * Initialize the chklog control block
   */
  memset ( local_recptr, 0, sizeof(chklog_record) );
  memcpy( (void *) &(local_recptr->eyecatcher), (void *) "chklog  ", 8 );
  local_recptr->fscklog_buf_length = FSCKLOG_BUFSIZE;
  local_recptr->fscklog_buf_ptr = fscklog_buffer;
  ...
  if( pi_rc == FSCK_OK ) {  /* the parms are good */
    ...
    if( pi_rc != FSCK_OK ) {  /*device open failed */
      ...
      if(pi_rc == FSCK_OK) {    /* a valid superblock */
        ...
        local_recptr->fscklog_agg_offset = local_recptr->ondev_fscklog_byte_offset;

        send_msg( fsck_SEPARATOR, 0 );
 ...
 * PARAMETERS:  none
 *
 * NOTES:  If this routine returns FSCK_OK then the superblock
...
retcode_t validate_superblock ( )
{
  ...
  if( vsb_rc != FSCK_OK ) { /* if read primary fails */
