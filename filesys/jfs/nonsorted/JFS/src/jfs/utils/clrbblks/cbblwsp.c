/* $Id: cbblwsp.c,v 1.1.1.1 2003/05/21 13:41:04 pasha Exp $ */
 ...
 *
 */
int32 alloc_wrksp ( int32, void  ** );
int32 alloc_wsp_extent ( int32 );
 ...
 */
int32 alloc_wrksp ( int32   length,
                    void  **addr_wrksp_ptr )
{
  int32 awsp_rc = 0;
  char *wsp_ptr = NULL;
  ...
  *addr_wrksp_ptr = NULL;      /* initialize return value */
  ...
  while ( (wsp_ptr == NULL) && (awsp_rc == 0) ) {
    ...
    if( (awsp_rc == 0) && (wsp_ptr == NULL) ) {  /*
                            ...
                            * storage yet
                            */
      awsp_rc = alloc_wsp_extent( min_length );  /* will allocate some
 ...
 *      failure: something else
 */
int32 alloc_wsp_extent ( int32 minimum_length )
{
  int32 awe_rc = 0;
...
int32 get_bdblk_record ( cbbl_bdblk_recptr *addr_bbrecptr )
{
  ...
  if( agg_recptr->free_bdblk_records != NULL ) {
	...
	(*addr_bbrecptr)->prev = NULL;
    	...
    	gbbr_rc = alloc_wrksp( bdblk_record_length, (void **) addr_bbrecptr );
    	}
	/*
	 * initialize fields 
	 */
