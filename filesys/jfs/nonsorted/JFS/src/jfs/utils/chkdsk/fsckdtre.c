/* $Id: fsckdtre.c,v 1.1.1.1 2003/05/21 13:39:16 pasha Exp $ */
 ...
 *                   CHANGE
 *
 *   PS - fix bug with NLS in file names
 *
			    ...
			    )
{
  ...
  left_key_len = left_key_num_unichars * sizeof(UniChar);
  right_key_len = right_key_num_unichars * sizeof(UniChar);

  if( right_key_len < left_key_len ) { /* right key is shorter */
    outcome = UniStrncmp( (void *) left_key,
		            (void *) right_key,
		            right_key_num_unichars );
...
//+++PS begin
  if( sb_ptr->s_flag & JFS_OS2 ) {  /* case is preserved but ignored */
    dkcs_rc = dTree_key_to_upper( left_key, &(ukey[0][0]),
				  (int32) left_key_len );
    if( dkcs_rc == FSCK_OK ) {
      dkcs_rc = dTree_key_to_upper( right_key, &(ukey[1][0]),
				    (int32) right_key_len );
      if( dkcs_rc == FSCK_OK ) {
	dkcs_rc = dTree_key_compare( &(ukey[0][0]), left_key_len,
				     &(ukey[1][0]), right_key_len,
				     &outcome );
	}
      }
